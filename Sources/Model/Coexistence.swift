...
enum Coexistence {
    ...
    // Provenance lives in `CoexistenceProvenance` (own file, no SwiftUI in its
    // dependency graph) so `Scripts/run-tests.sh` can compile the real rules.
    // These forwarders keep every existing call site unchanged.
    static func mergePreservingUserEntries(field: String, desired: [String], in existing: [String]) -> [String] {
        CoexistenceProvenance.mergePreservingUserEntries(field: field, desired: desired, in: existing)
    }
    static func isProtectiveExclusion(_ cidr: String) -> Bool {
        CoexistenceProvenance.isProtectiveExclusion(cidr)
    }
    static func newlyInjected(desired: [String], existingBefore: [String]) -> [String] {
        CoexistenceProvenance.newlyInjected(desired: desired, existingBefore: existingBefore)
    }
    static func injectedRecord(_ field: String) -> [String] {
        CoexistenceProvenance.injectedRecord(field)
    }
    static func commitProvenance(field: String, injected: [String]) {
        CoexistenceProvenance.commitProvenance(field: field, injected: injected)
    }
    static func withdraw(field: String, from existing: [String]) -> [String] {
        CoexistenceProvenance.withdraw(field: field, from: existing)
    }
}
