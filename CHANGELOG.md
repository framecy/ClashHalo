...
- **系统代理子系统局部重构（服务发现 / 应用验收 / 状态一致性 / 能力边界）**：
  按综合审查结论的 P0–P3 路线实施。
  - **共享服务选择 `ProxyServicePlan`**（`HelperProtocol.swift`，GUI/Helper/
    测试三方编译）：修复 `networksetup -listnetworkserviceorder` 解析中
    `(Hardware Port:` 分支不可达的控制流错误（该行同样以 `(` 开头，通用
    前缀分支先吞掉了它，服务与设备永远配不上对，选服务退化到全量启用
    列表）；禁用服务 `(*)` 不再产生配对；Helper 与 GUI fallback 现在用
    **同一套**目标服务枚举（活跃非虚拟服务，回退启用列表），不再一个
    配活跃子集、一个写全部启用服务；去掉「最多 2 个服务」上限。
  - **三态结果语义**：`SystemProxyApplyOutcome`（full / partial(ok:failed:) /
    failed）取代 anyOK 布尔——一个服务成功不再被当成系统代理已完整开启；
    仅 `.full` 映射为成功跨越 XPC。
  - **开启验收**：开启前 TCP 探活 mixed-port（未监听则拒绝开启，避免把
    系统指向死端口的断网事故）；开启后逐服务读回 HTTP/HTTPS/SOCKS 实际
    状态，部分失败时点名失败服务并给出 warn，不再静默乐观置位。
  - **状态复核**：`syncSystemProxyState` 从仅检查 HTTP 升级为 HTTP+HTTPS+
    SOCKS 三协议 + 端口全匹配；新增 `SystemProxyStatus`（off/active/partial
    + 目标/失败服务列表），菜单栏在 partial 时显示未覆盖服务。
  - **统一恢复入口 `restoreSystemProxy(reason:)`**：唤醒 / 网络恢复 /
    内核重启 / 内核切换 / TUN 挂起恢复全部走同一条「端口探活 → 写入 →
    复核」链路，删除所有 `systemProxyOn = true` 乐观写入。
  - **bypass 自愈对齐**：只探测/补齐真正承载本代理（HTTP 指向 127.0.0.1）
    的服务，不再对全部启用服务盲写 bypass。
  - **代理 DNS 事务（P2）**：系统 DNS 改指 127.0.0.1 前必须确认 mihomo
    DNS 已监听 53 且实际应答一次 DNS 探测（复用 TUN 数据面探针的报文
    格式）；重载失败或无应答则保持原 DNS 并告警，不再产生解析黑洞。
    探测为 3 次短重试（真机验证发现：重载后监听异步重建，单发 UDP 在
    就绪前发出即丢失，1.5s 等待窗无法挽回已丢的包）。
  - **能力边界说明（P3）**：网络页注明系统代理仅接管遵守代理设置的
    应用流量（UDP/QUIC 与绕过列表不经过内核，完整接管需 TUN）；连接页
    在系统代理模式（非 TUN）标注「仅统计进入 mihomo 的连接」。
  - **回归测试（P0-8）**：新增 `Tests/SystemProxy`（44 项）：解析分支顺序
    回归、目标服务选择（活性过滤/虚拟排除/无上限/回退）、`(*)` 禁用、
    `-get*proxy` 读回解析、三态分类、bypass 单一来源网段。
- **Helper 1.0.24 → 1.0.25（强制升级）**：旧 Helper 仍带不可达分支解析器、
  2 服务上限与 anyOK 语义，需升级才能让上述修复在特权路径生效。
- **Helper 1.0.25 → 1.0.26（强制升级）：退出/重启竞态误杀新内核**：
  真机复现——旧 App 退出触发 Helper 客户端死亡清理（kqueue + 2s 宽限），
  新 App 在宽限结束后 0.2s 才连上首个 XPC，入口接管守卫已放行，清理里的
  `killall mihomo` 兜底把新 App 刚拉起 2 秒的 root 内核杀掉，之后系统
  代理开关 7 次点击全部死在「mixed-port 未监听」。修复：清理在执行破坏
  性步骤（停内核/清代理/清路由）之前，用 `newerSessionOwnsState` 重查一次
  接管——现实探测（networksetup/pgrep fork）耗时恰好构成新客户端接入的
  时间窗。
- **系统代理模式下部分应用完全断网（fake-ip 黑洞，最终方案）**：
  代理模式把系统 DNS 指到 mihomo（127.0.0.1:53），而 mihomo 默认
  fake-ip——任何域名都解析成不可路由的 198.18.x 假地址。遵守系统代理的
  应用无恙（mihomo 反查假 IP），**不走代理的应用直连假地址 → 无 TUN 即
  无路由 → 彻底断网**（真机验证：直连假 IP 4s 超时，同域名走代理 200）。
  该重定向过去 12s 内就被「归属争夺」bug 静默回滚、从未真正驻留，修复
  归属后持续生效，故障随之显性化。最终方案：**代理模式不再重定向系统
  DNS**（`ensureProxyDNS` 在 fake-ip 下跳过并自愈残留重定向；域名防污染由
  mihomo sniffer 的 parse-pure-ip 承担）；**TUN 是系统 DNS 重定向的唯一
  属主**（有 TUN 时 198.18/15 有路由，fake-ip 属设计内），对账/回收逻辑
  相应回到单属主模型。redir-host 模式的配置仍走原有「监听确认+应答探测+
  快照回滚」事务。
- **同类问题系统审查与加固（防再发）**：以「系统级状态写点未断言安全
  前提 / 被掩盖机制显性化」为特征扫描全部网络状态写入面——
  - `enableTunnelDNS` 写点断言 fake-ip 网关接口存在（`mihomoTunInterface`，
    1.5s 缓存）：上游 tunOn 推导虽已含接口校验，但按教训写入函数自证
    前提，接口不存在时跳过并日志，待对账重试；
  - Helper 1.0.27：客户端死亡回收的 loopback 代理检查限定到「本 Helper
    最后写入的端口」，退出 ClashHalo 不再误清共存代理应用（Surge 等）
    的系统代理设置；Helper 重启失忆（端口未知）时退回任意-loopback
    匹配，保住断网兜底；
  - 审查结论：`restoreDNS()` 全量清空当前无调用方（无危害）；网关模式
    不改本机 DNS；路由注入/回收已有 PeerRouteGuard 判据与测试覆盖；
  - AGENTS 固化类规则：写点自证安全前提、修被掩盖机制先审设计意图、
    全局状态单一属主。
- **系统代理开关「陈旧 reachable」死巷（GUI）**：内核死亡但
  `reachable` 仍为 true 时，开启路径会跳过内核重启分支、直接死在端口
  验收门。修复：开启时若端口未监听，先复核控制器 API，确认内核已死则
  清除 `reachable` 并走正常重启流程（该场景原先会被端口门拦成反复失败，
  旧行为则是直接把系统指向死端口造成断网——门拦得对，但流程要能自愈）。
- **系统代理开启耗时优化（温路径 ~2.8s → ~1s）**：目标服务枚举加 15s
  缓存（一次开关里 Helper 写入与 GUI 验收共用，省 ~0.3–0.7s）；逐服务
  覆盖率读回复核降级为诊断路径——仅当 Helper 全量写入 Bool 与
  SCDynamicStore 三协议复核不一致时才跑（省一次枚举 + 每服务 3 次读回，
  ~0.5–1s）；合并陈旧状态检查与验收门的重复端口探测。验收语义不变：
  两项独立检查（写入全量 + 生效三协议）仍是开启的门槛，逐服务读回只
  负责在异常时点名失败服务。
...
- **系统代理开关静默失败（假成功）**：`setSystemProxyFallback` 的 shell 脚本用
  ...
  「系统代理已开启」，但 `scutil --proxy` 从未真正改变。改为 `for` 循环
  （不再是子 shell 里的 `while read`）显式统计每个网络服务应用成功的条数，
  只有真正生效才返回 0。同时给两处此前完全静默的失败路径补上日志：
  `callSystemProxy` 的 15s XPC 超时（此前无任何痕迹，直接无声降级到 shell fallback）、
  `runLocalShell` 的失败 stderr（此前只知道失败，不知道 networksetup 报了什么）。
- **内存警卫阈值再次校准**：softLimit 220→260MB / hardLimit 320→380MB。6 小时无系统代理监测的 RSS 均值 122.8MB、峰值 169.1MB，但用户开启系统代理后活跃连接涌入，RSS 合法地达到 220–235MB——这是正常使用不是泄漏。旧阈值在此场景下立即触发 3 次徒劳清理后进入退避，虽然实际已停止清空缓存，但日志文案「继续每 15 秒清空缓存」误导用户以为仍在空转。修正：阈值提至 260/380MB（高于活跃使用峰值），日志改为「已暂停自动缩减（不再清空缓存）」。
...
- **内存警卫阈值高于实际占用平台期，等于从不触发**：`recordHistoryOnly` 里的 App 内存警卫设在 **400MB**，而实测占用稳定在 350MB 附近，所以在 150–400MB 这整段区间里没有任何主动回收。阈值下调至 **250MB**（`appMemorySoftLimit`），高于健康空闲态（80–150MB）、低于观测到的平台期，使其在堆碎片化成型之前就介入而不是之后。

- **最活跃的连接页路径完全绕过内存警卫**：警卫此前只存在于 `recordHistoryOnly` 中，而连接页有自己的 1.5s 轮询器（`ConnectionsViewModel.onConnections`），是全应用分配最密集的路径——它每次 tick 读取 `residentMemoryBytes()` **仅用于显示**，从不据此做任何回收。现抽出共享的 `enforceAppMemoryGuard()`，由三个快照消费方共同调用：`recordHistoryOnly`、连接页视图模型、以及前台轮询循环。第三处补上的是「窗口可见 + 非连接页 + 网关关」这一最常见场景，此前没有任何消费方会在该场景下触发警卫。

- **警卫命中即整体清空，会把用户正在看的表格清空一拍**：现分两档。**软档**（250–400MB 且 UI 可见）只清关闭历史、把活动行截断到 300 条，表格保持有内容；**硬档**（>400MB，或 UI 不可见）才整体释放，UI 不可见时连 diff 记账一并释放。警卫内部按 15s 限频，避免 RSS 恰好卡在阈值上方时每秒反复抖动缓存。

- **连接热路径无 `autoreleasepool`，临时对象跨 tick 堆积**：`onConnections` 每 1.5s 为每条活动连接构造一个 `Conn`（14 个新分配的 `String`），外加字节字典、id 集合、网关聚合与仪表盘统计的暂存字典。这些临时对象此前一直挂在主运行循环的自动释放池上，直到下一次 `await` 才排空，于是多个 tick 的垃圾同时存活。现对 `onConnections`、`recordHistoryOnly` 的重活段落，以及 `fetchConnectionsSnapshot` 的 JSON 解码（全应用最大、最频繁的一次解码）显式加池。

- **`cachedConns` 无上限，且峰值容量长期占位**：连接数在下载 / P2P 场景可冲到数千，全量常驻。现加 `maxCachedConns = 2000` / `maxClosedConns = 200` 硬上限（`clampConnectionCaches()`），用 `Array(prefix:)` 重新分配以顺带释放连接峰值遗留的超额 capacity。截断前已按速率排序，被丢弃的**始终是空闲尾部**；UI 顶部计数取自 `activeConnectionsCount`，在截断之前赋值，**计数仍精确**。

### 已知行为变化

- 活动连接超过 2000 条时，连接表只渲染速率最高的 2000 行，超出部分不再进入「已关闭」列表。顶部计数不受影响。

### Tests

- `Scripts/run-tests.sh` **92/92 全部通过**。v1.1.15 记录的唯一失败项（`Tests/RouteGuard` 的 `localAttachedSubnets — 本机实测`，断言依赖运行机器实时网卡状态）在当前机器网络状态下亦已通过。
- 未新增测试：本版改动为分配行为与阈值调整，无法在不启动内核与真实连接流的前提下做确定性断言。内存回落效果需运行时观测（连接页高负载下看 `live.appMemoryMB` 与 kernel 日志中的「App 内存占用过高 …，已缩减/释放缓存」）。

### 未包含

- 工作区中一段未合并的 TUN `persistTunState` / flap 熔断器在途工作已剥离至 `wip/tun-persist-state` 分支，**不在本版内**，待在 1.1.15 基线上重新验证后再行合入。
