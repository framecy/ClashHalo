...
本版是内存警卫与仪表盘数据流的系统性修复，基于一次 2 小时真机监测 + 589 个真实样本的回放验证。**Helper 仍为 1.0.24，本版不需要重新授权。**

- **内存警卫度量口径修复**：从 `phys_footprint`（含压缩内存，~316MB 空载）切换为 RSS（`MACH_TASK_BASIC_INFO`，~90MB 空载），与 `ps -o rss` 和 Activity Monitor 一致。阈值按实测 RSS 分布重新校准：soft 220MB / hard 320MB（稳态平台期 146MB 的 1.5 倍 / 2.2 倍）。
- **徒劳退避**：连续 3 次清理未让 RSS 下降 10MB → 暂停，只在真实增长 50MB 或回落到软限以下时恢复。日志改为只在状态转换时输出。旧版在 2 小时内空转 463 次（每 15 秒一次，始终无效）；新版在同一负载下回放结果为 **0 次**。
- **仪表盘数据刷新解耦**：App 内存不再寄生在内核 `/memory` WebSocket 回调里（首帧 `inuse:0` 导致永久冻结），改为独立 2s 定时器采样。
- **可见性驱动刷新**：窗口和状态栏均不可见时停止所有 UI 数据刷新（保留健康巡检）；恢复可见时 2 秒内全量刷新（并发拉取代理/配置/连接快照）。
- **Keychain 授权弹窗修复**：`SecItemCopyMatching` 在 ACL 锁定的旧条目上会弹授权对话框，后台/无头启动时永远等不到。改用 `kSecUseAuthenticationUIFail` 抑制弹窗，文件镜像优先读取。旧版因此导致 `start()` 在主线程永久阻塞、仪表盘完全不刷新、内存飙升到 297MB；新版启动 0.9 秒完成。
